...
class RPMPlots(wx.Dialog):
    data = dict()
    labels = dict()
    listNdofA = ['density', 'velocity', 'pressure']
    listNdofB = ['density', 'pressure', 'temperature']
    cylExtras= ['convective heat-transfer coeff','radiactive heat-transfer coeff','convective heat-transfer rate','radiactive heat-transfer rate','burned mass fraction','burned mass fraction rate','mass flow rate trought intake port','mass flow rate trought exhaust port','mass of fuel','mass of air', 'mass of residual gas','total heat-transfer rate','fuel chemical energy release']
    tankExtras = ['mass flow rate','enthalpy flow rate','mass','heat-transfer rate']
    parsedData = dict()
    def __init__(self, *args, **kwds):
		# begin wxGlade: RPMPlots.__init__
    ...
    def __set_properties(self):
		# begin wxGlade: rpmPlots.__set_properties
		self.SetTitle("RPM Plots")
    ...
    def __do_layout(self):
		...
		grid_sizer_elementG = wx.FlexGridSizer(2, 2, 0, 0)
		grid_sizer_elementNG = wx.FlexGridSizer(3, 2, 0, 0)
		grid_sizer_element = wx.FlexGridSizer(2, 1, 0, 0)
		...
		grid_sizer_configure.Add(self.labels['cycle'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		grid_sizer_configure.Add(self.data['cycle'], 0, wx.ALIGN_CENTER_VERTICAL, 0)	
		grid_sizer_configure.Add(self.data['element'], 0, wx.ALIGN_CENTER_VERTICAL, 0)	

		grid_sizer_elementNG.Add(self.labels['element_number'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		grid_sizer_elementNG.Add(self.data['element_number'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		grid_sizer_elementNG.Add(self.labels['element_nnod'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		grid_sizer_elementNG.Add(self.data['element_nnod'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		grid_sizer_elementNG.Add(self.labels['element_ndof'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		grid_sizer_elementNG.Add(self.data['element_ndof'], 0, wx.ALIGN_CENTER_VERTICAL, 0)

		grid_sizer_elementG.Add(self.labels['global_variable'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		grid_sizer_elementG.Add(self.data['global_variable'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		#grid_sizer_elementG.Add(self.labels['global_variable_type'], 0, wx.ALIGN_CENTER_VERTICAL, 0)
		#grid_sizer_elementG.Add(self.data['global_variable_type'], 0, wx.ALIGN_CENTER_VERTICAL, 0)


		grid_sizer_element.Add(grid_sizer_elementNG)
		grid_sizer_element.Add(grid_sizer_elementG)
    ...
    def setParameters(self,plotData):
		...
		self.labels['cycle'] = wx.StaticText(self.panel_configure, -1, "Choose Cycle: ")
		self.data['cycle'] = wx.CheckListBox(self.panel_configure,choices=plotData['cycle'],size=(70,80))
		...
		self.labels['global_variable'] = wx.StaticText(self.panel_configure, -1, "Choose Global Variable: ")
		self.data['global_variable'] = wx.Choice(self.panel_configure, -1, choices=plotData['Globals']['labels'])
		...
		self.Bind(wx.EVT_CHOICE, self.onChangeGlobalVariable, self.data['global_variable'])
    ...
    def onChangeElement(self, event):
		...
		if elem == 0:
			...
			self.data['element_number'].Enable(False)
			self.labels['element_number'].SetLabel("- - -")
			self.labels['element_nnod'].SetLabel("- - -")
			self.labels['element_ndof'].SetLabel("- - -")
			self.data['global_variable'].Enable(True)
			#self.data['global_variable_type'].Enable(True)
			self.labels['global_variable'].SetLabel("Choose Global Variable: ")
			#self.labels['global_variable_type'].SetLabel("Choose Type: ")
		elif elem == 1:
			self.data['element_number'].Clear()
			self.data['element_nnod'].Clear()
			self.data['element_ndof'].Clear()
			...
			self.data['element_number'].Enable(True)
			self.data['element_nnod'].Enable(True)
			self.data['element_ndof'].Enable(True)
			self.data['global_variable'].Enable(False)
			#self.data['global_variable_type'].Enable(False)
			self.labels['global_variable'].SetLabel("- - -")
			#self.labels['global_variable_type'].SetLabel("- - -")
		elif elem == 2:
			self.data['element_number'].Clear()
			self.data['element_nnod'].Clear()
			self.data['element_ndof'].Clear()
    ...
    def onChangeGlobalVariable(self, event):
		ndof = self.data['global_variable'].GetStringSelection()
		self.data['units'].Clear()
		self.data['units'].AppendItems(possibleUnits[ndof])
